/* 遗传算法c.cpp : 定义控制台应用程序的入口点。
  本程序试用遗传算法来解决Rosenbrock函数的全局最大值计算问题：
...
const int M=8,T=100;        //  M 群体大小，pc 交叉概率,pm 变异概率,T 终止代数
...
void decoding(population *t)        //解码函数
{
	population *po;
	 int temp,s1=0,s2=0;
	 float m,n,dit;
	 n=ldexp(1.0,10);                  //n=2^10
	 dit=4.096/(n-1);                //dit=(U(max)-U(min))/n-1
	 for(po=t;po<t+M;po++) 
	 {
		 for(int i=0;i<10;i++)
		 {
			 temp=ldexp((float)(*po).x[i],i);   //ldexp(double value,int exp)=value*(2^exp)
			 s1=s1+temp;
		 }
		 m=-2.048+s1*dit;
		 (*po).x1=m;
